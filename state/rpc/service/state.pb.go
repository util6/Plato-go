...
func (x *StateResponse) GetCode() int32 {
	if x != nil { // 如果 x 不为空
		return x.Code // 返回 x 的 Code 字段
	}
	return 0 // 如果 x 为空，返回 0
}

func (x *StateResponse) GetMsg() string {
	if x != nil { // 如果 x 不为空
		return x.Msg // 返回 x 的 Msg 字段
	}
	return "" // 如果 x 为空，返回空字符串
}

var File_state_proto protoreflect.FileDescriptor // 定义 state.proto 文件的描述符

var file_state_proto_rawDesc = []byte{ // 定义 state.proto 文件的原始字节描述
    0x0a, 0x0b, 0x73, 0x74, 0x61, 0x74, 0x65, 0x2e, 0x70, 0x72, 0x6f, 0x74, 0x6f, 0x12, 0x07, 0x73,
    0x65, 0x72, 0x76, 0x69, 0x63, 0x65, 0x65, 0x22, 0x56, 0x0a, 0x0c, 0x53, 0x74, 0x61, 0x74, 0x65, 0x52,
